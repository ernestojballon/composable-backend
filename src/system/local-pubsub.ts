...
export class LocalPubSub {
  ...
  createTopic(topic: string): void {
    if (!util.validRouteName(topic)) {
      throw new Error(
        'Invalid topic name - use 0-9, a-z, period, hyphen or underscore characters',
      );
    }
    if (!topics.has(topic)) {
      topics.set(topic, []);
    ...
    platform.register(topic, new LocalPublisher(), 1, 'private', true);
  }
  ...
  unsubscribe(topic: string, memberRoute: string): void {
    if (this.topicExists(topic)) {
      ...
      if (members.includes(memberRoute)) {
        topics.set(
          topic,
          members.filter((m) => m != memberRoute),
        );
