...
async function handleStreamConsumer(
  evt: EventEnvelope,
  cid: string,
  readTimeout: number,
  replyTo: string,
...
export class ObjectStreamIO {
  ...
  constructor(expirySeconds = DEFAULT_TIMEOUT) {
    ...
    platform.register(this.streamIn, new StreamConsumer(), 1, 'private', true);
...
export class ObjectStreamReader {
  ...
  async read() {
    if (!this.eof) {
      const block = await po.request(
        new EventEnvelope().setTo(this.streamIn).setHeader(TYPE, READ),
        this.timeout,
      );
      if (DATA == block.getHeader(TYPE)) {
      ...
      if (END_OF_STREAM == block.getHeader(TYPE)) {
        ...
        await po.request(
          new EventEnvelope().setTo(this.streamIn).setHeader(TYPE, CLOSE),
          this.timeout,
        );
      }
    }
...
class StreamPublisher implements Composable {
  ...
  async handleEvent(evt: EventEnvelope) {
    const ctx = (evt as unknown as Record<string, unknown>)['_context'] as
      | { route?: string }
      | undefined;
    const id = getIdFromRoute(ctx?.route ?? null);
    if (id) {
      const stream = streams.get(id);
      if (stream) {
        let block: EventEnvelope = null;
        if (
          DATA == evt.getHeader(TYPE) &&
          !stream.eof_write &&
          evt.getBody() != null
        ) {
          block = new EventEnvelope()
            .setHeader(TYPE, DATA)
            .setBody(evt.getBody());
        }
        ...
        if (block) {
          ...
          stream.write_count++;
          const filename =
            TEMP_DIR + '/' + stream.id + '-' + stream.write_count;
...
class StreamConsumer implements Composable {
  ...
  async handleEvent(evt: EventEnvelope) {
    ...
    if (replyTo) {
      const ctx2 = (evt as unknown as Record<string, unknown>)['_context'] as
        | { route?: string }
        | undefined;
      const id = getIdFromRoute(ctx2?.route ?? null);
      const stream = streams.get(id);
      if (stream) {
