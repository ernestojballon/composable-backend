  ...
  save(
    ...
    if (!this.exists(route)) {
      if (
        'initialize' in that &&
        'handleEvent' in that &&
        that.initialize instanceof Function &&
        that.handleEvent instanceof Function
      ) {
        log.debug(`Loading ${that.constructor.name} as ${route}`);
        this.registry.save(route, that, instances, visibility, interceptor);
      } else {
        log.error(
          `Unable to load ${this.constructor.name} because it does not implement Composable`,
        );
      }
    }
  }
...
class SimpleRegistry {
  ...
  save(
    ...
    if (
      ...
      if (
        anyThat['inputSchema'] &&
        typeof (anyThat['inputSchema'] as { parse?: unknown }).parse ===
          'function'
      ) {
        meta['inputSchema'] = anyThat['inputSchema'];
      }
      if (
        anyThat['outputSchema'] &&
        typeof (anyThat['outputSchema'] as { parse?: unknown }).parse ===
          'function'
      ) {
