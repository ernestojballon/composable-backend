...
export class PostOffice {
  ...
  private loadContext(event: EventEnvelope) {
    const ctx = (event as unknown as Record<string, unknown>)['_context'] as
      | {
          route?: string;
          instance?: string;
          traceId?: string;
    ...
    if (ctx) {
      this.trackable = true;
      this.from = ctx.route ?? null;
      this.instance = ctx.instance ?? '1';
      this.traceId = ctx.traceId ?? null;
      this.tracePath = ctx.tracePath ?? null;
    } else {
      // Fallback: read from headers for backward compatibility
      const info = event.getHeaders();
      if ('my_route' in info) {
        this.trackable = true;
        this.from =
          typeof info['my_route'] == 'string' ? info['my_route'] : null;
      }
      if ('my_instance' in info) {
        this.instance =
          typeof info['my_instance'] == 'string' ? info['my_instance'] : null;
      }
      if ('my_trace_id' in info) {
        this.trackable = true;
        this.traceId =
          typeof info['my_trace_id'] == 'string' ? info['my_trace_id'] : null;
      }
      if ('my_trace_path' in info) {
        this.tracePath =
          typeof info['my_trace_path'] == 'string'
            ? info['my_trace_path']
            : null;
      }
    }
  ...
  /**
   * Fire-and-forget send using route and body
   *
   * @param route name of the target function
   * @param body payload to send
   */
  async emit(
    route: string,
    body: string | number | object | boolean | Buffer | Uint8Array,
  ): Promise<void> {
    const event = new EventEnvelope().setTo(route).setBody(body);
    await this.send(event);
  }

  /**
   * Make an asynchronous RPC call and return the response body directly
   *
   * @param route name of the target function
   * @param body payload to send
   * @param timeout value in milliseconds (default 60000)
   * @returns a future promise of the response body
   */
  async call(
    route: string,
    body: string | number | object | boolean | Buffer | Uint8Array,
    timeout = 60000,
  ): Promise<string | number | object | boolean | Buffer | Uint8Array> {
    const event = new EventEnvelope().setTo(route).setBody(body);
    const result = await this.request(event, timeout);
...
class PO {
  ...
  async send(event: EventEnvelope) {
    ...
    if (route) {
      const targetHttp = input.getHeader(X_EVENT_API)
        ? null
        : resolver.getEventHttpTarget(route);
      const headers = resolver.getEventHttpHeaders(route);
      if (targetHttp) {
        void this.sendEventApi(input, targetHttp, headers).catch((e) => {
          log.error(
            `Error in sending async event ${route} to ${targetHttp} - ${e.message}`,
          );
        });
        return;
      }
      if (handlers.has(route)) {
        if (SYSTEM_EVENT_SERVICES.includes(route)) {
          ...
          setImmediate(() => {
            ...
            Object.defineProperty(input, '_context', {
              value: {
                route,
                instance: '1',
                traceId: input.getTraceId() ?? null,
                tracePath: input.getTracePath() ?? null,
              },
              enumerable: false,
              configurable: true,
            });
      ...
      log.warn(
        `Event ${input.getId()} dropped because there is no target service route`,
      );
    }
  }
  ...
  async sendEventApi(
    ...
    const forwardEvent = new EventEnvelope(input.toMap()).setHeader(
      X_EVENT_API,
      eventApiType,
    );
    const evt = await this.remoteRequest(
      forwardEvent,
      targetHttp,
      headers,
      rpc,
    ...
    if (rpc) {
      ...
      evt
        ...
        .setTracePath(input.getTracePath())
        .setCorrelationId(input.getCorrelationId());
      try {
        await this.send(evt);
      } catch (e) {
        log.error(
          `Error in sending callback event ${route} from ${targetHttp} to ${callback} - ${e.message}`,
        );
      }
    } else if (evt.getStatus() != 202) {
      log.error(
        `Error in sending async event ${route} to ${targetHttp} - status=${evt.getStatus()}`,
      );
    }
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      if (route) {
        ...
        if (targetHttp) {
          ...
          this.remoteRequest(forwardEvent, targetHttp, headers, rpc)
            .then((res) => {
              resolve(res);
            })
            .catch((e) => {
          ...
          const timer = setTimeout(
            () => {
              ...
              reject(
                new AppException(
                  408,
                  `Route ${input.getTo()} timeout for ${timeout} ms`,
                ),
              );
            },
            Math.max(10, timeout),
          ...
          const map = {
            ...
            tracePath: input.getTracePath(),
            oid: input.getCorrelationId(),
            route: route,
            from: input.getFrom(),
          };
          ...
          reject(
            new AppException(
              404,
              `Event ${input.getId()} dropped because ${route} not found`,
            ),
          );
        }
      } else {
  ...
  remoteRequest(
    ...
    return new Promise((resolve, reject) => {
      ...
      req
        ...
        .setHeader('X-TTL', String(timeout))
        .setBody(bytes);
      let host: string;
      let path: string;
      try {
            ...
            try {
              ...
              resolve(
                new EventEnvelope()
                  .setStatus(400)
                  .setBody(
                    `Did you configure rest.yaml correctly? invalid result set - ${e.message}`,
                  ),
              );
            }
          } else if (
