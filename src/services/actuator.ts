...
export class ActuatorServices implements Composable {
  ...
  async handleEvent(evt: EventEnvelope) {
    ...
    if (payload && payload instanceof Object) {
      ...
      if (
        ...
        Object.keys(request.getHeaders()).length > 0
      ) {
  ...
  static async doRoutes() {
    ...
    for (const route of functionList) {
      ...
      const visibility = md['visibility'];
  ...
  static async doEnv() {
    ...
    result.setElement('origin', origin);
    const config = AppConfig.getInstance();
    const envVars = util.split(
      config.getProperty('show.env.variables', ''),
      ', ',
    );
    const propVars = util.split(
      config.getProperty('show.application.properties', ''),
      ', ',
    );
    const envKv = {};
    if (process) {
  ...
  static async doInfo() {
    ...
    result.setElement(
      'time.current',
      util.getLocalTimestamp(new Date().getTime()),
    );
  ...
  static async doHealthChecks() {
    ...
    for (const service of healthServices) {
      ...
      try {
        const infoReq = new EventEnvelope()
          .setTo(service)
          .setHeader(TYPE, INFO);
        ...
        const healthReq = new EventEnvelope()
          .setTo(service)
          .setHeader(TYPE, HEALTH);
    ...
    if (upstream.length == 0) {
      result['message'] =
        'Did you forget to define health.dependencies in application configuration?';
    }
    return new EventEnvelope()
      .setStatus(status)
      .setHeader(CONTENT_TYPE, APPLICATION_JSON)
