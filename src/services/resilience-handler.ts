...
export class ResilienceHandler implements Composable {
  @preload('resilience.handler', 100, 'private', true)
  ...
  async handleEvent(event: EventEnvelope) {
    if (
      event.getBody() instanceof Object &&
      event.getReplyTo() &&
      ...
      md.cumulative = Math.max(
        0,
        'cumulative' in input ? util.getInteger(input['cumulative']) : 0,
      );
      md.now = new Date().getTime();
      // Still in the backoff period?
      if (
        'backoff' in input &&
        that.handleBackoff(po, md, input, that, event)
      ) {
        return false;
      }
      ...
      const status = Math.max(
        200,
        'status' in input ? util.getInteger(input['status']) : 200,
      );
      ...
      if (
        'backoff_trigger' in input &&
        'backoff_seconds' in input &&
        that.triggerBackoff(po, md, input, that, event)
      ) {
        return false;
      }
  ...
  private makeDecision(
    po: PostOffice,
    md: ResilienceMetadata,
    input: object,
    that: ResilienceHandler,
    event: EventEnvelope,
    status: number,
  ) {
    ...
    const maxAttempt = Math.max(
      1,
      'max_attempts' in input ? util.getInteger(input['max_attempts']) : 1,
    );
    let attemptCount = Math.max(
      0,
      'attempt' in input ? util.getInteger(input['attempt']) : 0,
    );
    let delay = Math.max(
      10,
      'delay' in input ? util.getInteger(input['delay']) : 10,
    );
    // increment attempts
    attemptCount++;
  ...
  private triggerBackoff(
    ...
    const backoffTrigger = Math.max(
      1,
      'backoff_trigger' in input
        ? util.getInteger(input['backoff_trigger'])
        : 1,
    );
    const backoffSeconds = Math.max(
      1,
      'backoff_trigger' in input
        ? util.getInteger(input['backoff_seconds'])
        : 1,
    );
    md.cumulative++;
    if (md.cumulative > backoffTrigger) {
      // trigger backoff
      const waitPeriod =
        backoffSeconds + (backoffSeconds == 1 ? ' second' : ' seconds');
      md.result['decision'] = 2;
      md.result['status'] = 503;
      md.result['message'] =
        'Service temporarily not available - please try again in ' + waitPeriod;
