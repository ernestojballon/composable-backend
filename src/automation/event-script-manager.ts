...
export class EventScriptEngine {
  async start() {
    if (!started) {
      ...
      platform.register(TASK_EXECUTOR, new TaskExecutor(), 1, 'private', true);
      platform.register(
        HTTP_FLOW_ADAPTER,
        new HttpToFlow(),
        200,
...
export class EventScriptManager implements Composable {
  ...
  private async processRequest(event: EventEnvelope, flowId: string) {
    ...
    const po = new PostOffice(event);
    const flowInstance = this.getFlowInstance(
      event,
      flowId,
    ...
    const firstTask = new EventEnvelope()
      .setFrom(EVENT_MANAGER)
      .setTo(TASK_EXECUTOR)
      .setCorrelationId(flowInstance.id)
  ...
  private getFlowInstance(
    ...
    const flowInstance = new FlowInstance(
      flowId,
      cid,
      replyTo,
      template,
      event.getHeader(PARENT),
    );
