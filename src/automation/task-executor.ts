...
export class TaskExecutor implements Composable {
  ...
  initialize(): Composable {
    ...
    this.maxModelArraySize = util.str2int(
      config.getProperty('max.model.array.size', '1000'),
    );
    return this;
  }
  ...
  async handleEvent(event: EventEnvelope) {
    ...
    if (TIMEOUT in headers) {
      ...
      self.abortFlow(
        flowInstance,
        408,
        'Flow timeout for ' + flowInstance.getFlow().ttl + ' ms',
      );
      return false;
    }
    try {
      ...
      log.error(
        `Unable to execute flow ${flowName}:${flowInstance.id} - ${e.message}`,
      );
  ...
  private async checkCallBack(
    ...
    if (!from) {
      log.error(
        `Unable to process callback ${flowName}:${refId} - task does not provide 'from' address`,
      );
      return true;
    }
    const caller = from.includes('@')
    ...
    if (task) {
      ...
      log.error(
        `Unable to process callback ${flowName}:${refId} - missing task in ${caller}`,
      );
      return true;
    }
  ...
  private async handleException(
    ...
    const handler = isTaskLevel
      ? task.getExceptionTask()
      : flowInstance.getFlow().exception;
    /*
  ...
  private async inputDataMappingModelVar(
    isInput: boolean,
    lhs: string,
    rhs: string,
    source: MultiLevelMap,
  ...
  private setInputDataMappingValue(
    ...
    if (!valid) {
      ...
      log.error(
        `Invalid input mapping '${entry}' - expect: JSON, actual: ${type}`,
      );
    }
  }

  private inputDataMappingNormalCase(
    md: InputDataMappingMetadata,
    lhs: string,
    rhs: string,
    entry: string,
  ) {
  ...
  private async doInputDataMapping(
    md: InputDataMappingMetadata,
    entry: string,
    sep: number,
  ) {
    let lhs = this.substituteDynamicIndex(
      entry.substring(0, sep).trim(),
      md.source,
      false,
    );
    const rhs = this.substituteDynamicIndex(
      entry.substring(sep + 2).trim(),
      md.source,
      true,
    );
    const isInput =
      lhs.startsWith(INPUT_NAMESPACE) || lhs.toLowerCase() == INPUT;
    ...
    if (rhs.startsWith(EXT_NAMESPACE)) {
      ...
      await this.inputDataMappingModelVar(
        isInput,
        lhs,
        rhs,
        md.source,
  ...
  private async runNextFlow(
    ...
    if (!subFlow) {
      log.error(
        `Unable to process flow ${flowInstance.getFlow().id}:${flowInstance.id} - missing sub-flow ${task.functionRoute}`,
      );
    ...
    const forward = new EventEnvelope()
      .setTo(EVENT_MANAGER)
      .setReplyTo(TASK_EXECUTOR)
    ...
    const po = new PostOffice(
      new Sender(
        task.functionRoute,
        flowInstance.getTraceId(),
        flowInstance.getTracePath(),
      ),
    );
  ...
  private resolveDelayVar(
    ...
    if (d) {
      ...
      if (delay < flowInstance.getFlow().ttl) {
        ...
        log.warn(
          `Unable to schedule future task for ${task.service}` +
            ` because ${task.getDelayVar()} is invalid (TTL=${flowInstance.getFlow().ttl}, delay=${delay})`,
        );
      }
    } else {
      log.warn(
        `Unable to schedule future task for ${task.service} because ${task.getDelayVar()} does not exist`,
      );
    }
    return 0;
  }
  ...
  private async executeTask(
    ...
    error = {},
    dynamicListIndex = -1,
    dynamicListKey: string = null,
  ) {
    const task: Task = flowInstance.getFlow().tasks[processName];
    const valid = !!task;
    if (!valid) {
      log.error(
        `Unable to process flow ${flowInstance.getFlow().id}:${flowInstance.id} - missing task '${processName}'`,
      );
      this.abortFlow(
        flowInstance,
        500,
        SERVICE_AT + processName + ' not defined',
      );
      return;
    }
    // add the task to the flow-instance
    flowInstance.tasks.push(
      task.service == task.functionRoute
        ? task.service
        : `${task.service}(${task.functionRoute})`,
    );
    const combined = {};
  ...
  private outputDateMappingSetValue(
    ...
    if (rhs == OUTPUT_STATUS) {
      const status =
        typeof value == 'number' ? value : util.str2int(String(value));
    ...
    if (rhs == OUTPUT_HEADER) {
      if (!(value instanceof Object && !Array.isArray(value))) {
        const type = typeof value;
        log.error(
          `Invalid output mapping '${entry}' - expect: JSON, actual: ${type}`,
        );
        required = false;
      }
    }
  ...
  private async doOutputDataMapping(
    entry: string,
    sep: number,
    consolidated: MultiLevelMap,
    flowInstance: FlowInstance,
    ...
    const lhs = this.substituteDynamicIndex(
      entry.substring(0, sep).trim(),
      consolidated,
      false,
    );
    ...
    if (rhs.startsWith(FILE_TYPE)) {
      ...
      this.outputDateMappingSetValue(
        entry,
        consolidated,
        flowInstance,
        task,
  ...
  private handleCallBackFromForkAndJoin(
    ...
    log.debug(
      `Flow ${flowInstance.getFlow().id}:${flowInstance.id} fork-n-join #${seq} result ${callBackCount} of ${joinInfo.forks} from ${from}`,
    );
    if (callBackCount >= joinInfo.forks) {
      delete flowInstance.pipeMap[seq];
      log.debug(
        `Flow ${flowInstance.getFlow().id}:${flowInstance.id} fork-n-join #${seq} done`,
      );
  ...
  private handleCallbackFromPipeline(
    ...
    if (pipeline.isLastStep(n)) {
      ...
      log.debug(
        `Flow ${flowInstance.getFlow().id}:${flowInstance.id} pipeline #${seq} last step-${n + 1} ${pipeline.getTaskName(n)}`,
      );
    } else {
      log.debug(
        `Flow ${flowInstance.getFlow().id}:${flowInstance.id} pipeline #${seq} next step-${n + 1} ${pipeline.getTaskName(n)}`,
      );
    }
  ...
  private async handleCallback(
    ...
    for (const entry of mapping) {
      const sep = entry.lastIndexOf(MAP_TO);
      if (sep > 0) {
        await this.doOutputDataMapping(
          entry,
          sep,
          consolidated,
          flowInstance,
  ...
  private handleDecisionTask(
    ...
    if (decisionNumber < 1 || decisionNumber > nextTasks.length) {
      log.error(
        `Flow ${flowInstance.getFlow().id}:${flowInstance.id} ${task.service} returned invalid decision (${decisionValue})`,
      );
      this.abortFlow(
        flowInstance,
        500,
  ...
  private handleForkAndJoin(flowInstance: FlowInstance, task: Task): void {
    ...
    if (dynamicListKey && steps.length == 1) {
      ...
      if (Array.isArray(items)) {
        ...
        throw new Error(
          `Flow ${flowInstance.getFlow().id}:${flowInstance.id} ${task.service} - ${dynamicListKey} is not a list`,
        );
      }
    }
    if (steps.length > 0 && task.getJoinTask()) {
      this.executeForkAndJoin(
        flowInstance,
        task,
        steps,
        isList,
  ...
  private runPipeline(task: Task, map: MultiLevelMap): boolean {
    ...
    if (WHILE == task.getLoopType() && task.getWhileModelKey()) {
      ...
      return this.evaluateForCondition(
        map,
        task.comparator[0],
        task.comparator[1],
        task.comparator[2],
  ...
  private handlePipelineTask(
    ...
    if (task.pipelineSteps.length > 0) {
      if (this.runPipeline(task, map)) {
        ...
        log.debug(
          `Flow ${flowInstance.getFlow().id}:${flowInstance.id} pipeline #${seq} begin ${pipeline.getTaskName(0)}`,
        );
      ...
      if (flowInstance.replyTo) {
        ...
        if (status) {
          ...
          if (value > 0) {
            ...
            log.warn(
              `Unable to set status in response ${flowInstance.getFlow().id}:${flowInstance.id} - task ${task.service} return status is negative value`,
            );
          }
        }
  ...
  private pipelineCompletion(
    ...
    let iterate = false;
    if (
      WHILE == pipelineTask.getLoopType() &&
      pipelineTask.getWhileModelKey()
    ) {
      ...
      const v =
        typeof modelValue == 'number' ? modelValue : util.str2int(modelValue);
      ...
      iterate = this.evaluateForCondition(
        consolidated,
        pipelineTask.comparator[0],
        pipelineTask.comparator[1],
        pipelineTask.comparator[2],
      );
    }
    if (iterate) {
      pipeline.resetPointer();
      log.debug(
        `Flow ${flowInstance.getFlow().id}:${flowInstance.id} pipeline #${seq} first ${pipeline.getTaskName(0)}`,
      );
      this.executeTask(flowInstance, pipeline.getTaskName(0), seq);
    } else {
      delete flowInstance.pipeMap[seq];
  ...
  private evaluateForCondition(
    ...
    const value1 = modelVar1.startsWith(MODEL_NAMESPACE)
      ? mm.getElement(modelVar1)
      : modelVar1;
    const value2 = modelVar2.startsWith(MODEL_NAMESPACE)
      ? mm.getElement(modelVar2)
      : modelVar2;
    const v1 =
      typeof value1 == 'number' ? value1 : util.str2int(String(value1));
    const v2 =
      typeof value2 == 'number' ? value2 : util.str2int(String(value2));
    if (comparator == '<') {
      return v1 < v2;
    }
    if (comparator == '<=') {
  ...
  private setRhsElement(value, rhs: string, target: MultiLevelMap): void {
    ...
    if (colon != -1) {
      ...
      const matched = this.getValueByType(
        type,
        value,
        "RHS '" + rhs + "'",
        target,
  ...
  private async setConstantValue(
    lhs: string,
    rhs: string,
    target: MultiLevelMap,
  ) {
  ...
  private getConstantByType(lhs: string, last: number) {
    ...
    if (lhs.startsWith(BOOLEAN_TYPE)) {
      return (
        TRUE == lhs.substring(BOOLEAN_TYPE.length, last).trim().toLowerCase()
      );
    }
  ...
  private validateIndexBounds(ptr: number, text: string) {
    if (ptr > this.maxModelArraySize) {
      throw new Error(
        'Cannot set RHS to index > ' +
          ptr +
          ' that exceeds max ' +
          this.maxModelArraySize +
          ' - ' +
          text,
      );
    }
  ...
  private getValueWithSuffix(type: string, value) {
    ...
    throw new Error(
      `matching type must be substring(start, end), boolean, and, or, text, binary or b64`,
    );
  }
  ...
  private getConcatValue(command: string, value, data: MultiLevelMap) {
    ...
    if (parts.length == 0) {
      throw new Error(
        'parameters must be model variables and/or text constants',
      );
    } else {
      const sb = new StringBuilder();
  ...
  private getBooleanValue(command: string, value): boolean {
    ...
    if (filtered.length > 0 && filtered.length < 3) {
      ...
      const condition =
        filtered.length == 1 || 'true' == filtered[1].toLowerCase();
      if (str == filtered[0]) {
        return condition;
      } else {
        return !condition;
  ...
  private getValueByType(
    ...
    try {
      ...
      if (selection == OPERATION.SIMPLE_COMMAND) {
        ...
        const command = type
          .substring(type.indexOf('(') + 1, type.length - 1)
          .trim();
        /*
         * substring(start, end)]
         * substring(start)
        ...
        if (selection == OPERATION.SUBSTRING_COMMAND) {
          ...
          selection == OPERATION.AND_COMMAND ||
          selection == OPERATION.OR_COMMAND
        ) {
  ...
  private endFlow(flowInstance: FlowInstance, normal: boolean): void {
    ...
    annotations['execution'] =
      `Run ${totalExecutions} task${s} in ${formatted}`;
