...
export class CompileFlows {
  public start(): void {
    ...
    const locations = config.getProperty(
  ...
  private loadFlows(flowList: Array<string>, prefix: string) {
    ...
    for (const f of flowList) {
      ...
      if (filename.endsWith('.yml') || filename.endsWith('.yaml')) {
        ...
        log.error(
          `Ignored ${filename} because it does not have .yml or .yaml file extension`,
        );
      }
    }
  ...
  private isValidTaskConfiguration(
    ...
    if (!Array.isArray(input)) {
      log.error(
        `Unable to parse ${flowName} task ${taskIndex} - input must be a list`,
      );
      return false;
    }
    if (!Array.isArray(output)) {
      log.error(
        `Unable to parse ${flowName} task ${taskIndex} - output must be a list`,
      );
      return false;
    }
    if (!uniqueTaskName) {
      log.error(
        `Unable to parse ${flowName} task ${taskIndex} - task name must not be empty`,
      );
      return false;
    }
    if (typeof taskDesc != 'string' || taskDesc.length == 0) {
      log.error(
        `Unable to parse ${flowName} task ${taskIndex} - description must not be empty`,
      );
      return false;
    }
    if (typeof execution != 'string' || execution.length == 0) {
      log.error(
        `Unable to parse ${flowName} task ${taskIndex} - execution type must not be empty`,
      );
      return false;
    }
    if (!this.validExecutionType(execution)) {
      log.error(
        `Unable to parse ${flowName} task ${taskIndex} - execution type '${execution}' must be one of ${JSON.stringify(EXECUTION_TYPES)}`,
      );
      return false;
    }
    return true;
  ...
  private validateTaskName(
    ...
    if (
      uniqueTaskName.includes('://') &&
      !uniqueTaskName.startsWith(FLOW_PROTOCOL)
    ) {
      throw new Error(
        `Skip invalid task ${uniqueTaskName} in ${name}. Syntax is flow://{flow-name}`,
      );
    }
    if (
      functionRoute &&
      functionRoute.includes('://') &&
      !functionRoute.startsWith(FLOW_PROTOCOL)
    ) {
      throw new Error(
        `Skip invalid task process=${functionRoute} in ${name}. Syntax is flow://{flow-name}`,
      );
    }
    if (
      uniqueTaskName.startsWith(FLOW_PROTOCOL) &&
      ...
      throw new Error(
        `Skip invalid task process=${uniqueTaskName} in ${name}. process tag not allowed when name is a sub-flow`,
      );
    }
  }
  ...
  private addTask(
    ...
    if (
      this.parseInputDataMapping(flow, task, i, uniqueTaskName, name) &&
      this.parseOutputDataMapping(flow, task, i, uniqueTaskName, name)
    ) {
      entry.addTask(task);
    }
  }
  ...
  private parseInputDataMapping(
    ...
    for (let j = 0; j < input.length; j++) {
      inputList.push(
        flow.getProperty(TASKS + '[' + i + '].' + INPUT + '[' + j + ']'),
      );
    }
    let valid = true;
    ...
    for (const line of filteredInputMapping) {
      if (this.validInput(line)) {
        ...
        if (rhs.startsWith(INPUT_NAMESPACE) || rhs == INPUT) {
          log.warn(
            `Task ${uniqueTaskName} in ${name} uses input namespace in right-hand-side - ${line}`,
          );
        }
        task.input.push(line);
      } else {
        log.error(
          `Skip invalid task ${uniqueTaskName} in ${name} that has invalid input mapping - ${line}`,
        );
        valid = false;
      }
    }
  ...
  private parseOutputDataMapping(
    ...
    for (let j = 0; j < output.length; j++) {
      outputList.push(
        flow.getProperty(TASKS + '[' + i + '].' + OUTPUT + '[' + j + ']'),
      );
    }
    let valid = true;
    ...
    for (const line of filteredOutputMapping) {
      if (this.validOutput(line, isDecision)) {
        ...
        log.error(
          `Skip invalid task ${uniqueTaskName} in ${name} that has invalid output mapping - ${line}`,
        );
        valid = false;
      }
    }
    if (isDecision && task.nextSteps.length < 2) {
      log.error(
        `Skip invalid task ${uniqueTaskName} in ${name} must have at least 2 next tasks`,
      );
      valid = false;
    }
    return valid;
  ...
  private parseJoinTask(
    ...
    if (typeof join == 'string') {
      ...
      throw new Error(
        `Skip invalid task ${uniqueTaskName} in ${name}. Missing a join task`,
      );
    }
  }
  ...
  private parseNonSinkTask(
    ...
    if (!Array.isArray(nextTasks)) {
      throw new Error(
        `Skip invalid task ${uniqueTaskName} in ${name}. 'next' should be a list`,
      );
    }
    if (nextTasks.length == 0) {
      throw new Error(
        `Skip invalid task ${uniqueTaskName} in ${name}. Missing a list of next tasks`,
      );
    }
    if (
      nextTasks.length > 1 &&
      (SEQUENTIAL == execution || PIPELINE == execution)
    ) {
      throw new Error(
        `Skip invalid task ${uniqueTaskName} in ${name}. Expected one next task, Actual: ${nextTasks.length}`,
      );
    }
    if (nextTasks.length > 1 && source) {
      throw new Error(
        `Invalid ${execution} task ${uniqueTaskName} in ${name}.` +
  ...
  private parsePipeline(
    ...
    const loopStatement = flow.getProperty(
      TASKS + '[' + i + '].' + LOOP + '.' + STATEMENT,
    );
    const loopCondition = flow.get(
      TASKS + '[' + i + '].' + LOOP + '.' + CONDITION,
    );
    const pipelineList = flow.get(TASKS + '[' + i + '].' + PIPELINE, []);
    if (!Array.isArray(pipelineList)) {
      throw new Error(
        `Skip invalid task ${uniqueTaskName} in ${name}. 'pipeline' should be a list`,
      );
    }
    if (pipelineList.length == 0) {
      throw new Error(
        `Skip invalid task ${uniqueTaskName} in ${name}. Missing a list of pipeline tasks`,
      );
    }
    ...
    if (typeof loopCondition == 'string') {
      ...
      if (condition.length == 2) {
        ...
        throw new Error(
          `Skip invalid task ${uniqueTaskName} in ${name}. loop condition syntax error - ${loopCondition}`,
        );
      }
    }
    if (Array.isArray(loopCondition)) {
      for (const c of loopCondition) {
        ...
        if (condition.length == 2) {
          ...
          throw new Error(
            `Skip invalid task ${uniqueTaskName} in ${name}. loop conditions syntax error - ${error}`,
          );
        }
      }
    }
  ...
  private parseLoopStatement(
    ...
    if (bracket == -1) {
      throw new Error(
        `Skip invalid task ${uniqueTaskName} in ${name}. Please check loop.statement`,
      );
    }
    const type = loopStatement.substring(0, bracket).trim();
    if (type != FOR && type != WHILE) {
      throw new Error(
        `Skip invalid task ${uniqueTaskName} in ${name}. loop.statement must be 'for' or 'while'`,
      );
    }
    task.setLoopType(type);
    const parts: Array<string> = util.split(
      loopStatement.substring(bracket + 1),
      '(;)',
    );
    if (type == FOR) {
  ...
  private parseForLoop(
    ...
    if (parts.length < 2 || parts.length > 3) {
      throw new Error(
        `Skip invalid task ${uniqueTaskName} in ${name}. 'for' loop should have 2 or 3 segments`,
      );
    }
    if (parts.length == 2) {
    ...
    if (parts.length == 3) {
      ...
      if (initializer.length == 0) {
        throw new Error(
          `Skip invalid task ${uniqueTaskName} in ${name}. Please check for-loop initializer.` +
    ...
    if (task.comparator.length == 0 || task.sequencer.length == 0) {
      throw new Error(
        `Skip invalid task ${uniqueTaskName} in ${name}. Please check for-loop syntax.` +
          ' e.g. for (model.n = 0; model.n < 3; model.n++)',
      );
    }
    if (!this.validForStatement(task.comparator, task.sequencer)) {
      throw new Error(
        `Skip invalid task ${uniqueTaskName} in ${name}. 'for' loop has invalid comparator or sequencer`,
      );
    }
  }
  ...
  private parseWhileLoop(
    ...
    if (parts.length != 1) {
      throw new Error(
        `Skip invalid task ${uniqueTaskName} in ${name}. 'while' loop should have only one value`,
      );
    }
    const modelKey = parts[0].trim();
    if (
      !modelKey.startsWith(MODEL_NAMESPACE) ||
      modelKey.includes('=') ||
      modelKey.includes(' ')
    ) {
      throw new Error(
        `Skip invalid task ${uniqueTaskName} in ${name}. 'while' should use a model key`,
      );
    }
  ...
  private setDelay(
    ...
    if (util.isNumeric(delay)) {
      ...
      if (n < entry.ttl) {
        ...
        throw new Error(
          `Skip invalid task ${uniqueTaskName} in ${name}. delay must be less than TTL`,
        );
      }
    } else {
      ...
      if (dParts.length > 1 && delay.startsWith(MODEL_NAMESPACE)) {
        ...
        throw new Error(
          `Skip invalid task ${uniqueTaskName} in ${name}. delay variable must starts with 'model.'`,
        );
      }
    }
  }
  ...
  private addFlow(entry: Flow, name: string) {
    ...
    for (const t of taskList) {
      ...
      if (
        this.hasExternalState(task.input) ||
        this.hasExternalState(task.output)
      ) {
        extFound = true;
        break;
      }
    ...
    for (const t of taskList) {
      const task: Task = entry.tasks[t];
      if (
        this.hasIncompleteMapping(task.input) ||
        this.hasIncompleteMapping(task.output)
      ) {
        incomplete = true;
        break;
      }
    ...
    if (extFound && entry.externalStateMachine == null) {
      log.error(
        `Unable to parse ${name} - flow is missing external.state.machine`,
      );
    } else if (incomplete) {
  ...
  private getCondition(statement: string): string[] {
    ...
    if (
      parts.length == 3 &&
      parts[0] == 'if' &&
      parts[1].startsWith(MODEL_NAMESPACE) &&
  ...
  private getForPart1(text: string): string[] {
    ...
    if (
      parts.length == 3 &&
      parts[0].startsWith(MODEL_NAMESPACE) &&
      util.isNumeric(parts[2]) &&
      parts[1] == '='
    ) {
      result.push(parts[0]);
      result.push(parts[2]);
    ...
    for (const line of entries) {
      ...
      if (entry.startsWith(TEXT_TYPE)) {
        ...
        if (parts.length == 2) {
          ...
          result.push(
            'Syntax must be (LHS -> RHS) or (LHS -> model.variable -> RHS)',
          );
        }
      }
    }
  ...
  private handleThreePartFormat(result: Array<string>, parts: Array<string>) {
    ...
    if (
      parts[1].startsWith(MODEL_NAMESPACE) ||
      parts[1].startsWith(NEGATE_MODEL)
    ) {
      result.push(this.filterMapping(parts[0] + SPACED_MAP_TO + parts[1]));
      ...
      result.push(
        '3-part data mapping must have model variable as the middle part',
      );
    }
  }
  ...
  private normalizedNegateTypeMapping(negate: string): string {
    ...
    return (
      (negate.includes(':')
        ? negate.substring(1, negate.indexOf(':'))
        : negate.substring(1)) + ':!'
    );
  }
    ...
    if (sep > 0) {
      ...
      if (this.validModel(lhs) && this.validModel(rhs) && lhs != rhs) {
        if (
          ...
          lhs.startsWith(ERROR_NAMESPACE)
        ) {
          return true;
          ...
          return (
            (lhs.startsWith(TEXT_TYPE) ||
              lhs.startsWith(FILE_TYPE) ||
              lhs.startsWith(CLASSPATH_TYPE) ||
              lhs.startsWith(INTEGER_TYPE) ||
              lhs.startsWith(LONG_TYPE) ||
              lhs.startsWith(FLOAT_TYPE) ||
              lhs.startsWith(DOUBLE_TYPE) ||
              lhs.startsWith(BOOLEAN_TYPE)) &&
            lhs.endsWith(CLOSE_BRACKET)
          );
        }
      }
    }
  ...
  private validOutputLhs(lhs: string): boolean {
    if (
      ...
      lhs.startsWith(MODEL_NAMESPACE) ||
      lhs == RESULT ||
      lhs.startsWith(RESULT_NAMESPACE) ||
      lhs == STATUS ||
      lhs == HEADER ||
      lhs.startsWith(HEADER_NAMESPACE)
    ) {
      return true;
    } else if (lhs.startsWith(MAP_TYPE) && lhs.endsWith(CLOSE_BRACKET)) {
      return this.validKeyValues(lhs);
  ...
  private validOutputRhs(rhs: string, isDecision: boolean): boolean {
    return (
      (rhs == DECISION && isDecision) ||
      rhs.startsWith(FILE_TYPE) ||
      rhs.startsWith(OUTPUT_NAMESPACE) ||
