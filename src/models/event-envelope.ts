...
export class EventEnvelope {
  ...
  fromMap(map: object): this {
    ...
    if ('headers' in map) {
      ...
      this.headers =
        headers && headers.constructor == Object ? (headers as object) : {};
    }
    if ('body' in map) {
  ...
  private metadataFromMap(map: object) {
    ...
    if (typeof map['exec_time'] == 'number') {
      this.execTime = Math.max(
        0,
        util.getFloat(parseFloat(String(map['exec_time'])), 3),
      );
    }
    if (typeof map['round_trip'] == 'number') {
      this.roundTrip = Math.max(
        0,
        util.getFloat(parseFloat(String(map['round_trip'])), 3),
      );
    }
  }
  ...
  private bodyFromMap(map: object) {
    ...
    if (typeof map[EXECUTION_FLAG] == 'number') {
      this.execTime = Math.max(
        0,
        util.getFloat(parseFloat(String(map[EXECUTION_FLAG])), 3),
      );
    }
    if (typeof map[ROUND_TRIP_FLAG] == 'number') {
      this.roundTrip = Math.max(
        0,
        util.getFloat(parseFloat(String(map[ROUND_TRIP_FLAG])), 3),
      );
    }
  }
