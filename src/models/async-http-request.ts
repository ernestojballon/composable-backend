...
export class AsyncHttpRequest {
  ...
  setTargetHost(host: string): this {
    if (
      host &&
      (host.startsWith(HTTP_PROTOCOL) || host.startsWith(HTTPS_PROTOCOL))
    ) {
      this.targetHost = host;
      return this;
    } else {
      throw new Error(
        'Invalid host - must starts with ' +
          HTTP_PROTOCOL +
          ' or ' +
          HTTPS_PROTOCOL,
      );
  ...
  private fromMoreMap(map: object) {
    ...
    if (
      typeof map[TRUST_ALL_CERT] == 'boolean' ||
      typeof map[TRUST_ALL_CERT] == 'string'
    ) {
