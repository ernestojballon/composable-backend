...
export class Utility {
  ...
  getFloat(n: number, decimalPoint = 3): number {
    return n && n.constructor == Number
      ? parseFloat(n.toFixed(decimalPoint))
      : 0.0;
  }
  ...
  validRouteName(route: string): boolean {
    if (route) {
      ...
      for (let i = 0; i < route.length; i++) {
        if (
          (route.charAt(i) >= '0' && route.charAt(i) <= '9') ||
          (route.charAt(i) >= 'a' && route.charAt(i) <= 'z') ||
          route.charAt(i) == '.' ||
          route.charAt(i) == '_' ||
          route.charAt(i) == '-'
        )
  ...
  private createParentFolder(folder: string, path: string): void {
    if (fs.existsSync(folder)) {
      ...
      if (!file.isDirectory) {
        throw new Error(
          `Unable to create ${path} - ${folder} is not a directory`,
        );
      }
    } else {
      fs.mkdirSync(folder);
  ...
  public extractSegments(
    ...
    while (true) {
      ...
      const bracketEnd =
        bracketStart == -1 ? -1 : text.indexOf(end, bracketStart);
