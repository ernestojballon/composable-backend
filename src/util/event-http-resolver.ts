...
export class EventHttpResolver {
  ...
  loadHttpRoutes(file: string, config: ConfigReader) {
    if (!this.loaded) {
      ...
      if (Array.isArray(o)) {
        ...
        log.info(
          `Total ${total} event-over-http target${total == 1 ? '' : 's'} configured`,
        );
      } else {
        log.error(
          `Invalid config ${file} - the event.http section should be a list of route and target`,
        );
      }
    }
  }
    ...
    if (route && target) {
      ...
      if (h instanceof Object && !Array.isArray(h)) {
        ...
        Object.keys(h).forEach((k) => {
          headers[String(k)] = config.getProperty(
            'event.http[' + i + '].headers.' + k,
          );
          headerCount++;
        });
      ...
      log.info(
        `Event-over-HTTP ${route} -> ${target} with ${headerCount} header${headerCount == 1 ? '' : 's'}`,
      );
    }
  }
  ...
  getEventHttpHeaders(route: string): object {
    return this.eventHttpHeaders.has(route)
      ? this.eventHttpHeaders.get(route)
      : {};
  }
}
