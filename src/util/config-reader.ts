...
function overrideRunTime(config: ConfigReader, argv?: Array<string>) {
  ...
  const parameters = params.filter(
    (k) => k.startsWith('-D') && k.includes('='),
  );
...
function resolveResource(configFile: string): string {
  if (configFile.startsWith('classpath:')) {
    ...
    const cp = util.normalizeFilePath(
      configFile.substring('classpath:'.length),
    );
...
function getOtherResource(classPath: string): string {
  ...
  if (resourcePath.endsWith(TEST_RESOURCES)) {
    otherResources.push(
      resourcePath.substring(0, resourcePath.length - TEST_RESOURCES.length) +
        MAIN_RESOURCES,
    );
  }
  ...
  if (segments.length > 2) {
    ...
    if (packages) {
      ...
      for (const p of packageList) {
        otherResources.push(
          sb.getValue() + '/node_modules/' + p + '/dist/resources',
        );
      }
    }
  }
...
function getResourceFilePath(filePath: string): string {
  ...
  if (filePath.endsWith('.yaml') || filePath.endsWith('.yml')) {
    ...
    if (name) {
      const alternative = filePath.endsWith('.yaml')
        ? `${name}.yml`
        : `${name}.yaml`;
...
function loadConfigResource(
  configResource: string | object,
  ...
  if (configResource.constructor == Object) {
    ...
    log.error(
      `Configuration not loaded because config resource is not a file path or a JSON object`,
    );
...
function loadConfigFromFile(
  ...
  if (configResource.endsWith('.yaml') || configResource.endsWith('.yml')) {
    const filePath = isBaseConfig
      ? getResourceFilePath(configResource)
      : resolveResource(configResource);
    if (!filePath) {
...
export class AppConfig {
    ...
    if (!AppConfig.reader) {
      if (typeof resourcePath == 'string') {
        ...
        const discovered = candidates.find(
          (p) => fs.existsSync(p) && util.isDirectory(p),
        );
        if (discovered) {
          this.setResourcePath(discovered);
        } else {
          // no resource folder found — start with empty config
          AppConfig.reader = new ConfigReader({}, true);
        }
      }
    }
  }
  ...
  private setResourcePath(resourcePath: string) {
    AppConfig.reader = new ConfigReader(
      resourcePath + '/application.yml',
      true,
    );
    // save file path
  ...
  static getInstance(
    ...
    if (AppConfig.singleton === undefined) {
      ...
      const configSource =
        AppConfig.reader.getProperty('resource.path') ||
        'empty config (no resource folder found)';
      log.info(`Configuration loaded from ${configSource}`);
    }
...
export class ConfigReader {
  ...
  constructor(configResource?: string | object, isBaseConfig = false) {
    if (isBaseConfig) {
      ...
      if (ConfigReader.self === undefined) {
        throw new Error(
          'Cannot do user configuration because base configuration is not defined',
        );
      }
    ...
    if (configResource) {
      ...
      this.resolveEnvVars();
    } else if (isBaseConfig) {
      // allow base config to start empty (e.g. no resource folder found)
  ...
  private checkEnvVariables(
    ...
    for (const s of segments) {
      ...
      const evaluated = this.performEnvVarSubstitution(
        key,
        middle,
        defaultValue,
        loop,
  ...
  private performEnvVarSubstitution(
    ...
    if (text) {
      ...
      if (process && text in process.env) {
        ...
        const refs: Array<string> = this.loopDetection.has(loopId)
          ? (this.loopDetection.get(loopId) as Array<string>)
          : [];
