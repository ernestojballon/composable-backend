...
import {
  ClassScanUtility,
  ScannedComposable,
  scanComposableDefinitions,
} from './ts-class-scanner.js';

const util = new Utility();

function scanDecorator(
  content: string,
...
function scanPrototype(
  parent: string,
  filePath: string,
  md: DecoratorMetadata,
  clsParameters: object,
  clsMethods: object,
  clsMap: object,
  composables: object,
) {
    ...
    for (let i = 0; i < md.classBlocks.length; i++) {
      ...
      if (block.includes('.prototype') && preloadParams.length > 1) {
        ...
        if (elements.length > 1 && elements[0].endsWith('.prototype')) {
          const clsName = elements[0].substring(
            0,
  ...
  if (clsList.length > 0) {
    ...
    for (const clsName of clsList) {
      composables[clsName] = {
        ...
        file: relativePath,
        exportName: clsName,
        parameters: clsParameters[clsName] ?? [],
        method: clsMethods[clsName],
