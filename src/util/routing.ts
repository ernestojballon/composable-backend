...
export class CorsInfo {
  ...
  getOrigin(isOption: boolean): string {
    return isOption
      ? this.options.get(ACCESS_CONTROL_ORIGIN)
      : this.headers.get(ACCESS_CONTROL_ORIGIN);
  }
...
class RestEntry {
  ...
  load(config: ConfigReader): void {
    if (config.exists(HEADERS)) {
      ...
      if (Array.isArray(headers)) {
        ...
        log.error(
          "'headers' section must be a list of request and response entries",
        );
      }
    }
    if (config.exists(CORS)) {
      ...
      if (Array.isArray(cors)) {
        ...
        log.error(
          "'cors' section must be a list of Access-Control entries (id, options and headers)",
        );
      }
    }
    if (config.exists(REST)) {
      ...
      if (Array.isArray(rest)) {
        ...
        log.error(
          "'rest' section must be a list of endpoint entries (url, service, methods, timeout...)",
        );
      }
    }
  }
    ...
    if (this.routes.size > 0) {
      ...
      routeList.forEach((r) => {
        ...
        if (colon > 0) {
          ...
          if (
            !this.exactRoutes.has(urlOnly) &&
            !this.urlPaths.includes(urlOnly)
          ) {
  ...
  private loadHeaderTransform(config: ConfigReader, total: number): void {
    for (let i = 0; i < total; i++) {
      ...
      if (id != null) {
        ...
        log.error(
          `Skipping invalid header definition - Missing ${HEADERS}[${i}]`,
        );
      }
    }
  }

  private loadHeaderEntry(
    config: ConfigReader,
    idx: number,
    isRequest: boolean,
  ): void {
    ...
    log.info(
      `Loaded ${id}, ${type} headers, add=${counters.addCount}, drop=${counters.dropCount}, keep=${counters.keepCount}`,
    );
  }
  ...
  private addHeaders(
    ...
    for (let j = 0; j < addList.length; j++) {
      let valid = false;
      const kv = config.getProperty(
        HEADERS + '[' + idx + '].' + type + '.' + ADD + '[' + j + ']',
        'null',
      );
      ...
      if (!valid) {
        log.warn(
          `Skipping invalid header ${id} ${HEADERS}[${idx}].${type}.${ADD}`,
        );
      }
    }
  }
  ...
  private dropHeaders(
    ...
    for (let j = 0; j < dropList.length; j++) {
      const key = config.getProperty(
        HEADERS + '[' + idx + '].' + type + '.' + DROP + '[' + j + ']',
      );
      if (key) {
    ...
    for (let i = 0; i < total; i++) {
      ...
      if (id && Array.isArray(options) && Array.isArray(headers)) {
        if (this.validCorsList(options) && this.validCorsList(headers)) {
          ...
          log.error(
            `Skipping invalid cors entry id=${id}, options=${options}, headers=${headers}`,
          );
        }
      } else {
        log.error(
          `Skipping invalid cors definition ${config.get(CORS + '[' + i + ']')}`,
        );
      }
    }
  }
  ...
  private parseCors(config: ConfigReader, i: number) {
    ...
    for (let j = 0; j < options.length; j++) {
      info.addOption(
        config.getProperty(CORS + '[' + i + '].' + OPTIONS + '[' + j + ']'),
      );
    }
    for (let j = 0; j < headers.length; j++) {
      info.addHeader(
        config.getProperty(CORS + '[' + i + '].' + HEADERS + '[' + j + ']'),
      );
    }
  ...
  private validCorsElement(element: string): boolean {
    if (!element.startsWith(ACCESS_CONTROL_PREFIX)) {
      log.error(
        `cors header must start with ${ACCESS_CONTROL_PREFIX}, actual: ${element}`,
      );
      return false;
    }
    const colon = element.indexOf(':');
    if (colon == -1) {
      log.error(
        `cors header must contain key-value separated by a colon, actual: ${element}`,
      );
      return false;
    }
    const value = element.substring(colon + 1).trim();
  ...
  private loadRest(config: ConfigReader): void {
    ...
    for (let i = 0; i < total; i++) {
      ...
      if (
        ...
        (typeof services == 'string' || Array.isArray(services))
      ) {
        keys.push(`${url} | ${JSON.stringify(methods)} |${i}`);
      } else {
        log.error(
          `Skip invalid REST entry ${config.get(REST + '[' + i + ']')}`,
        );
      }
    }
  ...
  private addDefaultEndpoints(config: ConfigReader) {
    const defaultRest = util.loadYamlFile(
      util.getFolder('../resources/default-rest.yaml'),
    );
    ...
    if (missing.length > 0) {
      ...
      for (const entry of missing) {
        ...
        map.setElement(
          REST + '[' + total + ']',
          defaultRest.getElement(REST + '[' + idx + ']'),
        );
        total++;
      }
      config.reload(map);
  ...
  private loadRestEntry(
    ...
    let url = config
      .getProperty(REST + '[' + idx + '].' + URL_LABEL)
      .toLowerCase();
    try {
    ...
    if (corsId) {
      if (this.corsConfig.has(corsId)) {
        ...
        throw new Error(
          `cors ID ${corsId} not found, ${config.get(REST + '[' + idx + ']')}`,
        );
      }
    }
    ...
    if (info.primary.startsWith(HTTP) || info.primary.startsWith(HTTPS)) {
      ...
      const trustAll = config.getProperty(
        REST + '[' + idx + '].' + TRUST_ALL_CERT,
      );
      if (trustAll) {
        log.warn(
          `${TRUST_ALL_CERT} parameter for ${info.primary} is not relevant for regular service`,
        );
      }
    }
  ...
  private validateAuthConfig(
    ...
    if (typeof authConfig == 'string') {
      ...
      if (util.validRouteName(authConfig)) {
        ...
        throw new Error(
          `invalid authentication service name ${config.get(REST + '[' + idx + ']')}`,
        );
      }
    }
    if (Array.isArray(authConfig)) {
      ...
      if (info.defaultAuthService == null) {
        throw new Error(
          `missing default authentication service ${config.get(REST + '[' + idx + ']')}`,
        );
      }
    }
  }

  private validateHeaderTransformers(
    config: ConfigReader,
    idx: number,
    headerId: string,
    info: RouteInfo,
    ...
    if (!found) {
      throw new Error(
        `headers ID ${headerId} not found, ${config.get(REST + '[' + idx + ']')}`,
      );
    }
  }
  ...
  private parseUrlRewrite(config: ConfigReader, idx: number, info: RouteInfo) {
    ...
    if (Array.isArray(rewrite)) {
      if (rewrite.length == 2) {
        ...
        throw new Error(
          `invalid ${URL_REWRITE} - ${rewrite}. It should contain a list of 2 prefixes`,
        );
      }
    } else {
      throw new Error(
        `invalid ${URL_REWRITE} - ${rewrite}, expected: List<String>`,
      );
    }
    const u = new URL(info.primary);
    if (u.pathname != '/') {
      throw new Error(
        `invalid service URL ${info.primary} - Must not contain path`,
      );
    }
    if (u.search) {
      throw new Error(
        `invalid service URL ${info.primary} - Must not contain query`,
      );
    }
    const trustAll = config.getProperty(
      REST + '[' + idx + '].' + TRUST_ALL_CERT,
    ...
    if (info.primary.startsWith(HTTP) && trustAll != null) {
      log.warn(
        `${TRUST_ALL_CERT}=true for ${info.primary} is not relevant - Do you meant https?`,
      );
    }
  ...
  private loadAuthConfig(
    ...
    if (!valid) {
      log.error(
        `Skipping entry with invalid authentication service name ${config.get(REST + '[' + idx + ']')}`,
      );
    }
  }
    ...
    for (const s of parts) {
      ...
      if (!exact) {
        if (s.includes('*') && !this.validWildcard(s)) {
          ...
          s.includes('{') &&
          s.includes('}') &&
          !this.validArgument(s)
        ) {
          log.error(
            `invalid url path parameter inside curly brackets, actual ${s}`,
          );
          return null;
        }
      }
  ...
  addArguments(
    ...
    for (let i = 0; i < configured.length; i++) {
      ...
      if (configuredItem.startsWith('{') && configuredItem.endsWith('}')) {
        info.setArgument(
          configuredItem.substring(1, configuredItem.length - 1),
          urlParts[i],
        );
      }
    }
  ...
  matchRoute(
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      if (
        (configuredItem.startsWith('{') && configuredItem.endsWith('}')) ||
        '*' == configuredItem
      ) {
        continue;
      }
