...
function setMapElement(
  ...
  const pathname = compositePath.includes('[]')
    ? appendIndex(compositePath, map)
    : compositePath;
  ...
  if (pathname && !nullValue && map.constructor == Object) {
    ...
    if (
      list.length == 0 ||
      (list.length > 1 && updateMapElement(pathname, list, map, value))
    ) {
      return;
    }
...
function appendIndex(compositePath: string, map: object): string {
  ...
  if (emptyIndex != -1) {
    ...
    const result =
      compositePath.substring(0, emptyIndex) +
      '[' +
      findLastIndex(parent, map) +
      ']' +
