...
describe('config reader tests', () => {
  ...
  it('can parse rest.yaml', () => {
    ...
    expect(assigned.info.methods).toStrictEqual([
      'GET',
      'PUT',
      'POST',
      'HEAD',
  ...
  it('should throw an error when reading a folder as a configuration file', () => {
    ...
    expect(() => new ConfigReader(filePath)).toThrow(
      'Config file must not be a directory',
    );
    // this tests the Logger's warning log feature since we don't have a separate logger test
    log.warn(
      'Successfully throw exception when reading a folder as a config file',
    );
  ...
  it('should throw an error with invalid YAML extensions', () => {
    ...
    expect(() => new ConfigReader(filePath)).toThrow(
      'Config file must use .yml or .yaml extension',
    );
    // this tests some Logger features since we don't have a separate logger test
    log.error(
      'Successfully throw exception when reading YAML file with invalid extension',
  ...
  it('can resolve a file path from a resource path hierarchy', async () => {
    ...
    const filePath = config.resolveResourceFilePath(
      'classpath:\\templates\\preload.template',
    );
