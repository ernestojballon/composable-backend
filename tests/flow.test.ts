...
class SimpleAuth implements Composable {
  ...
  async handleEvent(evt: EventEnvelope) {
    ...
    return new EventEnvelope()
      .setBody(authorized)
      .setHeader('x-user-profile', 'demo-user-profile');
  }
}
...
function handleFileVaultBuffer(
  result: object,
  text: string,
  json: object,
  binary: Buffer,
...
class ExceptionSimulator implements Composable {
  ...
  async handleEvent(evt: EventEnvelope) {
    ...
    if ('accept' in input) {
      ...
      const attempt = util.str2int(
        'attempt' in input ? String(input['attempt']) : '0',
      );
      if (attempt == accept) {
...
class BreakableFunction implements Composable {
  ...
  async handleEvent(evt: EventEnvelope) {
    const input = evt.getBody() as object;
    const accept = util.str2int(
      'accept' in input ? String(input['accept']) : '0',
    );
    const attempt = util.str2int(
      'attempt' in input ? String(input['attempt']) : '0',
    );
    ...
    throw new AppException(
      400,
      'Just a demo exception for circuit breaker to handle',
    );
  }
}
...
class CircuitBreaker implements Composable {
  ...
  async handleEvent(evt: EventEnvelope) {
    const input = evt.getBody() as object;
    const maxAttempts = util.str2int(
      'max_attempts' in input ? String(input['max_attempts']) : '1',
    );
    const attempt = util.str2int(
      'attempt' in input ? String(input['attempt']) : '0',
    );
    const status = util.str2int(
      'status' in input ? String(input['status']) : '200',
    );
  ...
  async handleEvent(evt: EventEnvelope) {
    ...
    if (exceptionTag) {
      if (TIMEOUT == exceptionTag) {
        ...
        return new EventEnvelope()
          .setStatus(400)
          .setBody({ error: 'non-standard-format' });
      } else {
...
class HelloException implements Composable {
  ...
  async handleEvent(evt: EventEnvelope) {
    log.info(
      `User defined exception handler got ${JSON.stringify(evt.getHeaders())} ${JSON.stringify(evt.getBody())}`,
    );
    const input = evt.getBody() as object;
...
class ParallelTask implements Composable {
  ...
  async handleEvent(evt: EventEnvelope) {
    ...
    if (done) {
      log.info(
        'I am the second task so I will sleep briefly to let the first task to complete',
      );
...
describe('event flow use cases', () => {
  beforeAll(async () => {
    ...
    platform.register('greeting.test', new Greetings(), 5, 'public');
  ...
  it('can use external state machine function', async () => {
    ...
    const po = new PostOffice();
    const req1 = new AsyncHttpRequest()
      .setMethod('PUT')
      .setTargetHost(baseUrl)
      .setUrl(`/api/ext/state/${testUser}`)
      .setHeader('accept', 'application/json')
      .setHeader('content-type', 'application/json')
      .setBody(payload);
    const reqEvent1 = new EventEnvelope()
      .setTo(ASYNC_HTTP_CLIENT)
      .setBody(req1.toMap());
     ...
     */
    const req2 = new AsyncHttpRequest()
      .setMethod('GET')
      .setTargetHost(baseUrl)
      .setUrl(`/api/ext/state/${testUser}`)
      .setHeader('accept', 'application/json');
    const reqEvent2 = new EventEnvelope()
      .setTo(ASYNC_HTTP_CLIENT)
      .setBody(req2.toMap());
  ...
  it('can use external state machine flow', async () => {
    ...
    const req1 = new AsyncHttpRequest()
      .setMethod('PUT')
      .setTargetHost(baseUrl)
      .setUrl(`/api/ext/state/flow/${testUser}`)
      .setHeader('accept', 'application/json')
      .setHeader('content-type', 'application/json')
      .setBody(payload);
    const reqEvent1 = new EventEnvelope()
  ...
  it('can do type matching', async () => {
    ...
    expect(getByteArrayFromEncodedIntegers(map.getElement('binary'))).toEqual(
      helloWorldBytes,
    );
    expect(map.getElement('source.substring')).toBe(hello);
    expect(map.getElement('source.substring-2')).toBe(world);
    expect(
      getByteArrayFromEncodedIntegers(map.getElement('source.keep-as-binary')),
    ).toEqual(helloWorldBytes);
    expect(map.getElement('source.no-change')).toBe(helloWorld);
    expect(
      getByteArrayFromEncodedIntegers(map.getElement('source.binary')),
    ).toEqual(helloWorldBytes);
    expect(
      getByteArrayFromEncodedIntegers(map.getElement('source.bytes')),
    ).toEqual(helloWorldBytes);
  ...
  it('will abort with max attempts with a circuit breaker', async () => {
    const po = new PostOffice(
      new Sender('unit.test', '101000', 'TEST /circuit/break'),
    );
    const req = new AsyncHttpRequest()
      .setMethod('GET')
      .setTargetHost(baseUrl)
      .setUrl('/api/circuit/breaker/3')
      .setHeader('accept', 'application/json');
    const reqEvent = new EventEnvelope()
      .setTo(ASYNC_HTTP_CLIENT)
      .setBody(req.toMap());
  ...
  it('will handle backoff, retry and abort in resilience handler', async () => {
    ...
    // run test
    const po = new PostOffice(
      new Sender('unit.test', '101000', 'TEST /resilience'),
    );
    const req = new AsyncHttpRequest()
      .setMethod('GET')
      .setTargetHost(baseUrl)
      .setUrl('/api/resilience')
      .setQueryParameter('exception', '400')
      .setHeader('accept', 'application/json');
    const reqEvent = new EventEnvelope()
      .setTo(ASYNC_HTTP_CLIENT)
      .setBody(req.toMap());
    ...
    expect(map.getElement('message')).toBe(
      'Service temporarily not available - please try again in 2 seconds',
    );
    // Let the backoff period expires
    log.info('Making request during backoff period will throw exception 503');
    const requestDuringBackoff = new AsyncHttpRequest()
      .setMethod('GET')
      .setTargetHost(baseUrl)
      .setUrl('/api/resilience')
      .setQueryParameter('exception', '400')
      .setHeader('accept', 'application/json');
    const reqBo = new EventEnvelope()
      .setTo(ASYNC_HTTP_CLIENT)
      .setBody(requestDuringBackoff.toMap());
    ...
    const request1 = new AsyncHttpRequest()
      .setMethod('GET')
      .setTargetHost(baseUrl)
      .setUrl('/api/resilience')
      .setQueryParameter('exception', '401')
      .setHeader('accept', 'application/json');
    const reqEvent1 = new EventEnvelope()
      .setTo(ASYNC_HTTP_CLIENT)
      .setBody(request1.toMap());
    const result1 = await po.request(reqEvent1);
    expect(result1.getStatus()).toBe(200);
    expect(result1.getBody() instanceof Object);
    expect(result1.getBody()).toEqual({ path: 'alternative' });
    const request2 = new AsyncHttpRequest()
      .setMethod('GET')
      .setTargetHost(baseUrl)
      .setUrl('/api/resilience')
      .setQueryParameter('exception', '404')
      .setHeader('accept', 'application/json');
    const reqEvent2 = new EventEnvelope()
      .setTo(ASYNC_HTTP_CLIENT)
      .setBody(request2.toMap());
    const result2 = await po.request(reqEvent2);
  ...
  it('can do greeting test', async () => {
    ...
    expect(map.getElement('original.runtime_value3')).toBe(
    ...
    expect(map.getElement('original.unique_id1')).not.toEqual(
      map.getElement('original.unique_id2'),
    );
    expect(map.getElement('original.unique_id2')).toEqual(
      map.getElement('original.unique_id3'),
    );
  ...
  it('can do parent greeting test', async () => {
    ...
    const req = new AsyncHttpRequest()
      .setMethod('GET')
      .setTargetHost(baseUrl)
      .setUrl(`/api/parent-greeting/${testUser}`);
    const reqEvent = new EventEnvelope()
      .setTo(ASYNC_HTTP_CLIENT)
      .setBody(req.toMap());
    const result = await po.request(reqEvent);
  ...
  it('will reject when sub-flow is missing', async () => {
    ...
    const req = new AsyncHttpRequest()
      .setMethod('GET')
      .setTargetHost(baseUrl)
      .setUrl('/api/missing-flow/test-user')
      .setHeader('accept', 'application/json');
    const reqEvent = new EventEnvelope()
      .setTo(ASYNC_HTTP_CLIENT)
      .setBody(req.toMap());
  ...
  it('can throw exception from user function', async () => {
    ...
    const req = new AsyncHttpRequest()
      .setMethod('GET')
      .setTargetHost(baseUrl)
      .setUrl('/api/greetings/test-user')
  ...
  it('will reject invalid decision value', async () => {
    ...
    const map = new MultiLevelMap(result.getBody() as object);
    expect(map.getElement('message')).toBe(
      'Task numeric.decision returned invalid decision (100)',
    );
    expect(map.getElement('status')).toBe(500);
    expect(map.getElement('type')).toBe('error');
  });
  ...
  it('can do sequential tasks', async () => {
    ...
    const req = new AsyncHttpRequest()
      .setMethod('GET')
      .setTargetHost(baseUrl)
      .setUrl('/api/sequential/test-user')
  ...
  it('can do a delayed response task', async () => {
    ...
    const req1 = new AsyncHttpRequest()
      .setMethod('GET')
      .setTargetHost(baseUrl)
      .setUrl('/api/delayed-response/test-user')
      .setQueryParameter('seq', '100')
      .setHeader('accept', 'application/json');
    // Since there are only 3 items in the next tasks, a decision value of 100 is invalid
    const reqEvent = new EventEnvelope()
  ...
  it('can do fork-n-join', async () => {
    ...
    const req1 = new AsyncHttpRequest()
      .setMethod('GET')
      .setTargetHost(baseUrl)
      .setUrl('/api/fork-n-join/test-user')
      .setQueryParameter('seq', '100')
      .setHeader('accept', 'application/json');
    // Since there are only 3 items in the next tasks, a decision value of 100 is invalid
    const reqEvent = new EventEnvelope()
  ...
  it('can do dynamic-fork-n-join', async () => {
    ...
    const req1 = new AsyncHttpRequest()
      .setMethod('GET')
      .setTargetHost(baseUrl)
      .setUrl('/api/fork-n-join-with-dynamic-model/test-user')
      .setQueryParameter('seq', '100')
      .setHeader('accept', 'application/json');
    // Since there are only 3 items in the next tasks, a decision value of 100 is invalid
    const reqEvent = new EventEnvelope()
  ...
  it('can do fork-n-join-flows', async () => {
    ...
    const req1 = new AsyncHttpRequest()
      .setMethod('GET')
      .setTargetHost(baseUrl)
      .setUrl('/api/fork-n-join-flows/test-user')
      .setQueryParameter('seq', '100')
      .setHeader('accept', 'application/json');
    // Since there are only 3 items in the next tasks, a decision value of 100 is invalid
    const reqEvent = new EventEnvelope()
  ...
  it('can do for-loop in pipeline', async () => {
    ...
    const currentRoute = mock
      .assignFunctionRoute('echo.one', 'my.mock.function')
      .getFunctionRoute('echo.one');
    ...
    expect(map.getElement('content')).toBe(
  ...
  it('can do for-loop with single task in pipeline', async () => {
    ...
    const req1 = new AsyncHttpRequest()
      .setMethod('GET')
      .setTargetHost(baseUrl)
      .setUrl('/api/for-loop-single/test-user')
      .setQueryParameter('seq', '100')
      .setHeader('accept', 'application/json');
    // the iterationCount will be incremented by "my.mock.function"
    iterationCount = 0;
    ...
    const currentRoute = mock
      .assignFunctionRoute('item.picker', 'my.mock.function')
      .getFunctionRoute('item.picker');
    expect(previousRoute).toBe('no.op');
    expect(currentRoute).toBe('my.mock.function');
    const reqEvent = new EventEnvelope()
      .setTo(ASYNC_HTTP_CLIENT)
  ...
  it('can do for-loop with break in pipeline - case 1', async () => {
    ...
    const req1 = new AsyncHttpRequest()
      .setMethod('GET')
      .setTargetHost(baseUrl)
      .setUrl('/api/for-loop-break/test-user')
      .setQueryParameter('seq', '100')
      .setQueryParameter('break', '2')
      .setHeader('accept', 'application/json');
    // Since there are only 3 items in the next tasks, a decision value of 100 is invalid
    const reqEvent = new EventEnvelope()
      .setTo(ASYNC_HTTP_CLIENT)
  ...
  it('can do for-loop with break in pipeline that has a single task - case 3', async () => {
    ...
    const req1 = new AsyncHttpRequest()
      .setMethod('GET')
      .setTargetHost(baseUrl)
      .setUrl('/api/for-loop-break-single/test-user')
      .setQueryParameter('seq', '100')
      .setQueryParameter('none', '2')
      .setHeader('accept', 'application/json');
    // Since there are only 3 items in the next tasks, a decision value of 100 is invalid
    const reqEvent = new EventEnvelope()
      .setTo(ASYNC_HTTP_CLIENT)
  ...
  it('can do for-loop with continue in pipeline', async () => {
    ...
    const req1 = new AsyncHttpRequest()
      .setMethod('GET')
      .setTargetHost(baseUrl)
      .setUrl('/api/for-loop-continue/test-user')
      .setQueryParameter('seq', '100')
      .setHeader('accept', 'application/json');
    // Since there are only 3 items in the next tasks, a decision value of 100 is invalid
    const reqEvent = new EventEnvelope()
  ...
  it('can handle pipeline exception', async () => {
    ...
    const req1 = new AsyncHttpRequest()
      .setMethod('GET')
      .setTargetHost(baseUrl)
      .setUrl('/api/pipeline-exception/test-user')
      .setQueryParameter('seq', '100')
      .setHeader('accept', 'application/json');
    // Since there are only 3 items in the next tasks, a decision value of 100 is invalid
    const reqEvent = new EventEnvelope()
  ...
  it('can do while-loop in pipeline', async () => {
    ...
    const req1 = new AsyncHttpRequest()
      .setMethod('GET')
      .setTargetHost(baseUrl)
      .setUrl('/api/while-loop/test-user')
      .setQueryParameter('seq', '100')
      .setHeader('accept', 'application/json');
    // Since there are only 3 items in the next tasks, a decision value of 100 is invalid
    const reqEvent = new EventEnvelope()
  ...
  it('can do while-loop with break in pipeline', async () => {
    ...
    const req1 = new AsyncHttpRequest()
      .setMethod('GET')
      .setTargetHost(baseUrl)
      .setUrl('/api/while-loop-break/test-user')
      .setQueryParameter('seq', '100')
      .setHeader('accept', 'application/json');
    // Since there are only 3 items in the next tasks, a decision value of 100 is invalid
    const reqEvent = new EventEnvelope()
  ...
  it('can do while-loop with continue in pipeline', async () => {
    ...
    const req1 = new AsyncHttpRequest()
      .setMethod('GET')
      .setTargetHost(baseUrl)
      .setUrl('/api/while-loop-continue/test-user')
      .setQueryParameter('seq', '100')
      .setHeader('accept', 'application/json');
    // Since there are only 3 items in the next tasks, a decision value of 100 is invalid
    const reqEvent = new EventEnvelope()
  ...
  it('can do parallel tasks', async () => {
    ...
    expect(result.getBody()).toEqual({
      key1: 'hello-world-one',
      key2: 'hello-world-two',
    });
  });
