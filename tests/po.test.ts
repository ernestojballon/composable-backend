...
import {
  ...
  ObjectStreamReader,
  ObjectStreamWriter,
} from '../src/system/object-stream';
import { EventHttpResolver } from '../src/util/event-http-resolver';
import { HttpToFlow } from '../src/adapters/http-to-flow';
...
async function performRawHttpGet(
  path: string,
...
class HelloDownload implements Composable {
  ...
  async handleEvent(evt: EventEnvelope) {
    ...
    return new EventEnvelope()
      .setHeader(STREAM_CONTENT, stream.getInputStreamId())
...
class CustomContentTypeSender implements Composable {
  ...
  async handleEvent(evt: EventEnvelope) {
    return new EventEnvelope()
      .setHeader(
        'content-type',
        'application/vnd.my.org-v2.0+json; charset=utf-8',
      )
      .setBody(evt.getBody());
  }
}
...
class DemoInterceptor implements Composable {
  ...
  async handleEvent(evt: EventEnvelope) {
    const ctx = (evt as Record<string, unknown>)['_context'] as
      | { route?: string }
      | undefined;
    const myRoute = ctx?.route ?? null;
    //
    ...
    if (replyTo) {
      ...
      const response = new EventEnvelope()
        .setBody(evt.getBody())
        .setTo(replyTo)
      ...
      log.info(
        `Interceptor ${myRoute} does not respond because there is no reply_to address`,
      );
    }
    return null;
  }
...
describe('post office use cases', () => {
  beforeAll(async () => {
    ...
    const configMap = {
      'event.api.url': 'http://127.0.0.1:${server.port:8080}/api/event',
      'base.url': 'http://127.0.0.1:${server.port:8080}',
    };
    ...
    server.setupMiddleWare(async function (
      _req: Request,
      _res: Response,
      next: NextFunction,
    ) {
  ...
  it('can retrieve metadata from Composable function', async () => {
    const po = new PostOffice(
      new Sender('unit.test', '101', 'TEST /demo/metadata'),
    );
    const req = new EventEnvelope()
      .setTo(HELLO_WORLD_SERVICE)
      .setHeader('type', 'metadata');
    const result = await po.request(req, 3000);
  ...
  it('can send to an interceptor and expect a result', async () => {
    const cid = 'abc001';
    const po = new PostOffice(
      new Sender('unit.test', '120', 'TEST /demo/interceptor'),
    );
    const req = new EventEnvelope()
      .setTo(HELLO_INTERCEPTOR_SERVICE)
      .setBody(TEST_MESSAGE)
  ...
  it('can catch exception from RPC', async () => {
    ...
    const po = new PostOffice(
      new Sender('unit.test', '200', 'TEST /demo/exception'),
    );
  ...
  it('can catch timeout from RPC', async () => {
    const req = new EventEnvelope()
      .setTo(HELLO_WORLD_SERVICE)
      .setHeader(TYPE, TIMEOUT);
    // Send a call to trigger an exception
    const po = new PostOffice(
      new Sender('unit.test', '300', 'TEST /demo/timeout'),
    );
    await expect(po.request(req, 100)).rejects.toEqual(
      new AppException(408, 'Route hello.world timeout for 100 ms'),
    );
    // Check the exact exception status and message with the try-await-catch pattern
    let normal = false;
    try {
  ...
  it('can emit a fire-and-forget event without error', async () => {
    const po = new PostOffice();
    await expect(
      po.emit(HELLO_WORLD_SERVICE, TEST_MESSAGE),
    ).resolves.toBeUndefined();
  });

  it('can call a service and get the body directly', async () => {
    const po = new PostOffice();
    const body = await po.call(HELLO_WORLD_SERVICE, TEST_MESSAGE);
    expect(body).toBe(TEST_MESSAGE);
  });

  it('can call a service with a custom timeout and get the body directly', async () => {
    const po = new PostOffice();
    const body = await po.call(HELLO_WORLD_SERVICE, TEST_MESSAGE, 3000);
    expect(body).toBe(TEST_MESSAGE);
  });
  ...
  it('can handle async HTTP request', async () => {
    ...
    httpRequest
      ...
      .setTargetHost(host)
      .setBody(TEST_MESSAGE)
      .setHeader(contentType, textPlain)
      ...
      .setPathParameter(user, name);
    const req = new EventEnvelope()
      .setTo(HELLO_BFF_SERVICE)
      .setBody(httpRequest.toMap());
    const po = new PostOffice();
    const result = await po.request(req, 3000);
    expect(result.getBody()).toBeInstanceOf(Object);
  ...
  it('can catch exception from a HTTP service', async () => {
    ...
    const po = new PostOffice();
    await expect(po.request(req, 3000)).rejects.toEqual(
      new AppException(500, NOT_HTTP_REQUEST),
    );
  });
  ...
  it('can reply to a callback', async () => {
    ...
    const po = new PostOffice(
      new Sender('unit.test', '333', 'TEST /demo/callback'),
    );
    // send a request to hello.world and set the replyTo address as my.callback
    const request = new EventEnvelope()
      .setTo(HELLO_WORLD_SERVICE)
      .setBody(TEST_MESSAGE)
      .setReplyTo(MyCallBackOne.name);
    po.send(request);
  ...
  it('can catch all traces with a distributed trace forwarder', async () => {
    ...
    const po = new PostOffice(
      new Sender('unit.test', TraceForwarder.traceId, TraceForwarder.tracePath),
    );
    const request = new EventEnvelope()
      .setTo(HELLO_WORLD_SERVICE)
      .setBody(TEST_MESSAGE);
  ...
  it('can deliver events orderly', async () => {
    ...
    for (let i = 1; i <= TEST_CYCLES; i++) {
      const request = new EventEnvelope()
        .setTo(HELLO_WORLD_SERVICE)
        .setBody(TEST_MESSAGE + ' ' + i)
        .setReplyTo(MyCallBackTwo.name);
      po.send(request);
    }
    ...
    for (let i = 1; i <= MyCallBackTwo.count; i++) {
      ...
      const evtCtx = (event as Record<string, unknown>)?.['_context'] as
        | { route?: string; instance?: string }
        | undefined;
      expect(evtCtx?.route).toBe(MyCallBackTwo.name);
      expect(evtCtx?.instance).toBe('1');
      ...
      log.info({
        received: { header: event?.getHeaders(), body: event?.getBody() },
      });
  ...
  it('can handle custom content type mapping', async () => {
    const po = new PostOffice(
      new Sender('unit.test', '80000', 'POST /api/hello/custom/content/type'),
    );
    const req = new AsyncHttpRequest()
      .setMethod('POST')
      .setTargetHost(baseUrl)
      .setUrl('/api/hello/custom/content/type')
      .setHeader('content-type', 'application/json')
      .setHeader('accept', 'application/json')
      .setBody({ hello: 'world' });
    const reqEvent = new EventEnvelope()
      .setTo(ASYNC_HTTP_CLIENT)
      .setBody(req.toMap());
  ...
  it('can make a RPC call using Event API', async () => {
    const po = new PostOffice(
      new Sender('unit.test', 'A10000', 'TEST /api/event/test'),
    );
    const event = new EventEnvelope()
      .setTo(HELLO_WORLD_SERVICE)
      .setBody('test')
      .setHeader('x', 'y');
    const result = await po.remoteRequest(event, endApiUrl, {
      authorization: 'demo',
    });
  ...
  it('can reject a RPC call with HTTP-401 using Event-over-HTTP API', async () => {
    const po = new PostOffice(
      new Sender('unit.test', 'B10000', 'TEST /api/event/test'),
    );
    const event = new EventEnvelope()
      .setTo(HELLO_WORLD_SERVICE)
      .setBody('test')
  ...
  it('can make a drop-n-forget call using Event-over-HTTP API', async () => {
    const po1 = new PostOffice(
      new Sender('unit.test', '10001a', 'TEST /api/event/test'),
    );
    const event = new EventEnvelope()
      .setTo(HELLO_WORLD_SERVICE)
      .setBody('test')
    ...
    const po = new PostOffice(
      new Sender('unit.test', '10001b', 'TEST /api/event/test'),
    );
    const result = await po.remoteRequest(
      event,
      endApiUrl,
      { authorization: 'demo' },
      false,
  ...
  it('will throw exception when the remote service is private', async () => {
    const po = new PostOffice(
      new Sender('unit.test', '10003', 'TEST /api/event/test'),
    );
    const event = new EventEnvelope()
      .setTo(HELLO_PRIVATE_SERVICE)
      .setBody('test');
    const result = await po.remoteRequest(event, endApiUrl, {
      authorization: 'demo',
    });
  ...
  it('will throw exception when the remote service does not exist', async () => {
    const po = new PostOffice(
      new Sender('unit.test', '10004', 'TEST /api/event/test'),
    );
  ...
  describe('platform edge cases', () => {
    it('register() throws for non-Composable object', () => {
      const route = `edge.non.composable.${Date.now()}`;
      expect(() => platform.register(route, {})).toThrow(/not a Composable/);
    });

    it('register() throws for route with uppercase characters', () => {
      expect(() =>
        platform.register('Invalid.Route', new HelloWorld()),
      ).toThrow(
        'Check route name - use 0-9, a-z, period, hyphen or underscore characters',
      );
    });

    it('registerComposable(null) throws', () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      expect(() => platform.registerComposable(null as any)).toThrow(
        'Composable definition is required',
      );
    });

    it('registerComposable(undefined) throws', () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      expect(() => platform.registerComposable(undefined as any)).toThrow(
        'Composable definition is required',
      );
    });

    it('isPrivate() returns true for non-existent route', () => {
      expect(platform.isPrivate(`no.such.route.${Date.now()}`)).toBe(true);
    });

    it('release() is a no-op for non-existent route', () => {
      expect(() =>
        platform.release(`no.such.route.${Date.now()}`),
      ).not.toThrow();
    });

    it('exists() returns false for non-existent route', () => {
      expect(new PostOffice().exists(`no.such.route.${Date.now()}`)).toBe(
        false,
      );
    });
  });
  ...
  it('can subscribe to a pub/sub topic', async () => {
    ...
    platform.register(member1, new DemoInterceptor(), 1, 'private', true);
    platform.register(member2, new DemoInterceptor(), 1, 'private', true);
    const cid = 'x201';
    const po = new PostOffice(
      new Sender('unit.test', '222', 'TEST /demo/pubsub'),
    );
    const req = new EventEnvelope()
      .setTo(topic)
      .setBody(TEST_MESSAGE)
  ...
  it('can get response from /health endpoint', async () => {
    ...
    expect(map.getElement('dependency[0].message')).toEqual({
  ...
  it('can do HTTP-GET to /api/hello/world service', async () => {
    ...
    const req = new AsyncHttpRequest()
      .setMethod('GET')
      .setTargetHost(baseUrl)
      .setUrl('/api/hello/world?a=b&c=d');
  ...
  it('can do HTTP-PUT to send text to /api/hello/world service', async () => {
    ...
    jsonString = jsonString.substring(
      0,
      jsonString.length - HTML_SUFFIX.length,
    );
    ...
    expect(result.getHeader('access-control-allow-methods')).toBe(
      'GET, DELETE, PUT, POST, PATCH, OPTIONS',
    );
  ...
  it('can retrieve a function from the registry and use it', async () => {
    ...
    const po = new PostOffice(
      new Sender('unit.test', '400', 'TEST /library/rpc'),
    );
    const req = new EventEnvelope()
      .setTo(DEMO_LIBRARY_FUNCTION)
      .setBody(TEST_MESSAGE);
    const result = await po.request(req, 3000);
    expect(result.getBody()).toBe(TEST_MESSAGE);
  });
  ...
  it('can re-register a service', async () => {
    ...
    let result = await po.request(
      new EventEnvelope().setTo(serviceName).setBody(TEST_MESSAGE),
    );
    expect(result.getBody()).toBe(TEST_MESSAGE);
    ...
    for (let i = 0; i < 10; i++) {
      result = await po.request(
        new EventEnvelope().setTo(serviceName).setBody(TEST_MESSAGE + i),
      );
  ...
  it('can do HTTP-GET with JSON to /api/hello/world service', async () => {
    const request = new AsyncHttpRequest()
      .setMethod('GET')
      .setTargetHost(baseUrl)
    ...
    expect(result.getHeader('strict-transport-security')).toBe(
      'max-age=31536000',
    );
  ...
  it('preserves reserved characters in query parameters', async () => {
    ...
    expect(result.getBody()).toBeInstanceOf(Object);
    const map = new MultiLevelMap(result.getBody() as object);
    expect(map.getElement('parameters.query.message')).toBe(
      'hello & world = yes',
    );
  ...
  it('can do HTTP-POST multipart upload to /api/hello/upload service', async () => {
    ...
    expect(
      map
        .getElement('headers.content-type')
        .startsWith('multipart/form-data; boundary='),
    ).toBe(true);
    expect(map.getElement('filename')).toBe('hello.txt');
    expect(
      map.getElement('headers.' + STREAM_CONTENT).startsWith('stream.'),
    ).toBe(true);
  ...
  it('turns missing upload streams into a normal error response', async () => {
    ...
    expect(result).toBeTruthy();
    expect(result.getStatus()).toBe(404);
    expect(String(result.getBody())).toContain(
      'stream.missing-upload.in not found',
    );
  });
  ...
  it('contains async event-over-http delivery failures', async () => {
    ...
    try {
      const po = new PostOffice(
        new Sender('unit.test', 'A10001', 'TEST /api/event/async-failure'),
      );
      await expectNoUnhandledRejection(async () => {
  ...
  it('can detect HTTP connection exception from AsyncHttpClient', async () => {
    ...
    const reqEvent = new EventEnvelope()
      ...
      .setBody(request.toMap());
    const po = new PostOffice(
      new Sender('unit.test', '1200', 'TEST /not/reachable'),
    );
    const result = await po.request(reqEvent, 3000);
    expect(result.getHeader('content-type')).toBe('application/json');
    ...
    expect(map.getElement('message')).toBe(
      'Connection refused - 127.0.0.1:60800',
    );
  });
  ...
  it('can download data from /api/hello/download', async () => {
    ...
    const reqEvent = new EventEnvelope()
      ...
      .setBody(request.toMap());
    const po = new PostOffice(
      new Sender('unit.test', '2500', 'TEST /download/stream'),
    );
    const result = await po.request(reqEvent, 3000);
  ...
  it('can do fork-n-join to /api/hello/world service', async () => {
    ...
    for (const response of result) {
      ...
      expect(response.getHeader('expires')).toBe(
  ...
  it('validates schema input and transforms body through parse()', async () => {
    const po = new PostOffice(
      new Sender('unit.test', '900', 'TEST /schema/ok'),
    );
    const req = new EventEnvelope()
      .setTo(SCHEMA_VALIDATED_SERVICE)
  ...
  it('rejects invalid input with AppException 400', async () => {
    const po = new PostOffice(
      new Sender('unit.test', '901', 'TEST /schema/bad-input'),
    );
    const req = new EventEnvelope()
      .setTo(SCHEMA_VALIDATED_SERVICE)
  ...
  it('rejects invalid output with AppException 500', async () => {
    const po = new PostOffice(
      new Sender('unit.test', '902', 'TEST /schema/bad-output'),
    );
    const req = new EventEnvelope()
      .setTo(SCHEMA_VALIDATED_SERVICE)
  ...
  it('PostOffice with no arguments returns null context values', () => {
    const po = new PostOffice();
    expect(po.getMyRoute()).toBeNull();
    // instance field defaults to "1" in the constructor; getMyInstance() is not gated on trackable
    expect(po.getMyInstance()).toBe('1');
    expect(po.getMyTraceId()).toBeNull();
  });

  it('parallelRequest rejects when called with an empty array', async () => {
    const po = new PostOffice();
    await expect(po.parallelRequest([], 3000)).rejects.toMatchObject({
      status: 400,
      message: 'Input must be not an empty list',
    });
  });

  it('remoteRequest rejects when endpoint uses an unsupported protocol', async () => {
    const po = new PostOffice();
    const event = new EventEnvelope()
      .setTo(HELLO_WORLD_SERVICE)
      .setBody('test');
    await expect(
      po.remoteRequest(event, 'ftp://example.com/api/event'),
  ...
  it('remoteRequest rejects when endpoint is a malformed URL', async () => {
    const po = new PostOffice();
    const event = new EventEnvelope()
      .setTo(HELLO_WORLD_SERVICE)
      .setBody('test');
    await expect(
      po.remoteRequest(event, 'not-a-valid-url-###'),
    ).rejects.toMatchObject({
      status: 400,
    });
  });

  it('PostOffice reads context from old-style headers when _context property is absent', () => {
    const routeName = `legacy.route.${Date.now()}`;
    const instanceNum = '3';
    const traceId = 'trace-legacy-001';
    const evt = new EventEnvelope();
    evt.setHeader('my_route', routeName);
    evt.setHeader('my_instance', instanceNum);
    evt.setHeader('my_trace_id', traceId);
    // No _context property on evt — forces the fallback header-reading branch in loadContext()
    const po = new PostOffice(evt);
    expect(po.getMyRoute()).toBe(routeName);
    expect(po.getMyInstance()).toBe(instanceNum);
    expect(po.getMyTraceId()).toBe(traceId);
  });

  it('http-to-flow returns HTTP 400 when x-flow-id header is missing', async () => {
    const HTTP_FLOW_TEST = 'http.flow.adapter.test';
    platform.register(HTTP_FLOW_TEST, new HttpToFlow());
    const po = new PostOffice();
    const request = new AsyncHttpRequest()
      .setMethod('GET')
      .setTargetHost(baseUrl)
      .setUrl('/api/hello/world');
    // Intentionally omit x-flow-id header — processRequest throws AppException(400, ...)
    // HttpToFlow catches the exception and sends back an error envelope via po.send (not setException),
    // so the promise resolves rather than rejects.
    const reqEvent = new EventEnvelope()
      .setTo(HTTP_FLOW_TEST)
      .setBody(request.toMap());
    const result = await po.request(reqEvent, 3000);
    expect(result.getStatus()).toBe(400);
    ...
    expect(String(body['message'])).toContain('Missing x-flow-id');
  });

  it('async-http-client returns error when target host is missing', async () => {
    const po = new PostOffice();
    const request = new AsyncHttpRequest()
      .setMethod('GET')
      .setUrl('/api/hello/world');
    // No setTargetHost — setupHttpRequest throws AppException(400, 'Missing target host...')
    const reqEvent = new EventEnvelope()
      .setTo(ASYNC_HTTP_CLIENT)
      .setBody(request.toMap());
    await expect(po.request(reqEvent, 3000)).rejects.toEqual(
      new AppException(400, 'Missing target host. e.g. https://hostname'),
    );
  });

  it('async-http-client returns error for invalid protocol (ftp://)', async () => {
    const po = new PostOffice();
    // AsyncHttpRequest.setTargetHost() rejects ftp:// at the model layer,
    // so we inject the invalid host directly into the raw map to reach the
    // protocol guard inside setupHttpRequest() (async-http-client.ts lines 165-172).
    const rawRequest = new AsyncHttpRequest()
      .setMethod('GET')
      .setUrl('/api/hello/world')
      .toMap();
    rawRequest['host'] = 'ftp://example.com';
    const reqEvent = new EventEnvelope()
      .setTo(ASYNC_HTTP_CLIENT)
      .setBody(rawRequest);
    await expect(po.request(reqEvent, 3000)).rejects.toEqual(
      new AppException(400, 'Protocol must be http or https'),
    );
  });

  it('async-http-client returns error for invalid HTTP method', async () => {
    const po = new PostOffice();
    const request = new AsyncHttpRequest()
      .setMethod('INVALID')
      .setTargetHost(baseUrl)
      .setUrl('/api/hello/world');
    // setupHttpRequest throws AppException(405, 'Method not allowed')
    const reqEvent = new EventEnvelope()
      .setTo(ASYNC_HTTP_CLIENT)
      .setBody(request.toMap());
    await expect(po.request(reqEvent, 3000)).rejects.toEqual(
      new AppException(405, 'Method not allowed'),
    );
  });
});
